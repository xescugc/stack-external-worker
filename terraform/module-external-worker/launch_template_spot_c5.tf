# Temporarly create a c5 template to simplify rollback if spot type are not available in the region

variable "worker_type_c5" {
  default = "c5.2xlarge"
}

resource "aws_launch_template" "worker_c5" {
  name_prefix = "${var.project}_c5_${var.env}_version_"

  image_id      = "${data.aws_ami.worker.id}"
  instance_type = "${var.worker_type_c5}"
